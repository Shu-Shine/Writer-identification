...
def dictionary(descriptors, n_clusters):
    ...
    kmeans = MiniBatchKMeans(n_clusters=n_clusters)  # k=100
...
def vlad(files, mus, powernorm, gmp=False, gamma=1000):
    ...
    for f in tqdm(files):
        ...
        for k in range(mus.shape[0]):
            ...
            # Select only descriptors assigned to the current cluster k
            descriptors_cluster_k = desc[np.where(a[:, k] == 1)]

            # Compute the residual by subtracting the cluster center
            residual = descriptors_cluster_k - mus[k]

            if gmp:
                # Perform generalized max pooling
                pass
            else:
                # Perform sum pooling
                vlad_k = np.sum(residual, axis=0)

            # Accumulate the encodings for each cluster
            f_enc[k * D: (k + 1) * D] = vlad_k
...
def esvm(encs_test, encs_train, C=1000):  # Regularization parameter C
    """ 
    ...
    # set up labels
    # TODO
    labels = np.ones(len(encs_train) + 1)
    ...
    def loop(i):
        ...
        join = np.vstack((encs_train, encs_test[i]))
        clf.fit(join, labels)

        # get the weights
        w = clf.coef_
        # normalize them
        x = normalize(w, norm='l2')
...
if __name__ == '__main__':
    set_start_method('spawn')  # this is needed for the parallel map to work
    ...
    if not os.path.exists('mus.pkl.gz'):
        # TODO
        descriptors = loadRandomDescriptors(files_train, 500000)  # 500000 descriptors
        print('> loaded {} descriptors:'.format(len(descriptors)))

        # cluster centers
        print('> compute dictionary')
        # TODO
        mus = dictionary(descriptors, 100)  # k=100
        print('> computed dictionary of size {}'.format(len(mus)))
    ...
    fname = 'enc_test_gmp{}.pkl.gz'.format(args.gamma) if args.gmp else 'enc_test.pkl.gz'  #args.gamma?????
    if not os.path.exists(fname) or args.overwrite:
        # TODO
        # compute VLAD for test
        # without normalization
        enc_test = vlad(files_test, mus, powernorm=True, gamma=args.gamma)
    ...
    if not os.path.exists(fname) or args.overwrite:
        ...
        enc_train = vlad(files_train, mus, powernorm=True, gamma=args.gamma)
